...
class DescripcionactividadMeditacionWidget extends StatefulWidget {
  const DescripcionactividadMeditacionWidget({
    Key key,
  ...
  @override
  _DescripcionactividadMeditacionWidgetState createState() =>
      _DescripcionactividadMeditacionWidgetState();
}

class _DescripcionactividadMeditacionWidgetState
    extends State<DescripcionactividadMeditacionWidget> {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<AppointmentsRecord>(
      ...
      builder: (context, snapshot) {
        ...
        final descripcionactividadMeditacionAppointmentsRecord = snapshot.data;
        return Scaffold(
          key: scaffoldKey,
          appBar: AppBar(
                  ...
                  children: [
                    StreamBuilder<AppointmentsRecord>(
                      stream: AppointmentsRecord.getDocument(
                          descripcionactividadMeditacionAppointmentsRecord
                              .reference),
                      builder: (context, snapshot) {
                        // Customize what your widget looks like when it's loading.
                        if (!snapshot.hasData) {
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              image: DecorationImage(
                                fit: BoxFit.cover,
                                image: Image.asset(
                                  'assets/images/40469879-0FE6-472F-BA1C-EA204C6EBC0F.png',
                                ).image,
                              ),
                              borderRadius: BorderRadius.circular(8),
                              border: Border.all(
